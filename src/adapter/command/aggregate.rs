...
pub fn short_to_uuid(short: &str) -> Result<String, WorkflowError> {
    ...
    for &b in short.as_bytes() {
        let val = match b {
            ...
            b'A'..=b'Z' => b - b'A' + 10,
            b'a'..=b'z' => b - b'a' + 36,
            _ => return Err(StorageError::NotFound(short.to_string()).into())
        };
        num = num
            .checked_mul(62)
            .and_then(|n| n.checked_add(val as u128))
            .ok_or_else(|| WorkflowError::from(ValidationError::InvalidState("ID too long".to_string())))?;
    }
...
pub async fn load_aggregate_data(
    ...
    if events.is_empty() {
        return Err(StorageError::NotFound(aggregate_id.to_string()).into());
    }
...
impl Command for DeleteAggregateCommand {
    ...
    async fn load(
        ...
        let events = app_context.event_store.get_events(&full_id).await?;
        if events.is_empty() {
            return Err(StorageError::NotFound(self.aggregate_id.clone()).into());
        }
