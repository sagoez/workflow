...
fn copy_default_translations(i18n_dir: &Path) -> Result<()> {
    ...
    fs::write(&en_file, en_content).with_context(|| {
        format!(
            "Failed to write English translations: {}",
            en_file.display()
        )
    })?;
    ...
    fs::write(&es_file, es_content).with_context(|| {
        format!(
            "Failed to write Spanish translations: {}",
            es_file.display()
        )
    })?;
        ...
        for entry in entries.flatten() {
            if let Some(file_name) = entry.file_name().to_str() {
                if file_name.ends_with(".yaml") || file_name.ends_with(".yml") {
                    if let Some(lang_code) = file_name
                        .strip_suffix(".yaml")
